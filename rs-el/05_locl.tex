...
\begin{frame}[label=current]{Static vs Dynamic Routing}
	\begin{center}
		\begin{tikzpicture}
			...
			\node              (net2) [right=0.5cm of r2,visible on=<2->] {192.168.2.0/24};
			\node              (net3) [right=0.5cm of r3,visible on=<7->] {192.168.3.0/24};
		...
		\end{tikzpicture}
	\end{center}
	\texttt{\pause\pause R1(config)\#
	      \pause\\ip route 192.168.2.0 255.255.255.0 10.0.12.2
	      \pause\\ip route 192.168.2.0 255.255.255.0 10.0.13.3\pause~2\pause
...
\begin{frame}[label=current]{Static vs Dynamic Routing (Cont.)}
